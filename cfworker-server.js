...
// Bark通知配置
const BARK_URL = "";                  // 在此填写Bark的推送URL(例如: https://api.day.app/你的Key/)，或留空使用其他方式配置，可选择外置变量BARK_URL

// Google两步验证配置
const GOOGLE_AUTH_SECRET = "";        // 在此填写Google Authenticator的密钥，或留空使用其他方式配置，可选择外置变量GA_SECRET
...
async function verifyTOTP(secret, token) {
    ...
    try {
        ...
        const timeSteps = [Math.floor(epoch / 30), Math.floor(epoch / 30) - 1, Math.floor(epoch / 30) + 1]; // 容错前后30秒
...
export default {
    ...
    async scheduled(event, env, ctx) {
      try {
        await checkAndNotifyExpired订阅(env);
      } catch (error) {
...
// ==========================================
  ...
  async function handleAPI(request, env) {
    ...
    // API鉴权
    const authRequired = await isAuthRequired(env);
    if (authRequired && path !== '/check-setup') {
      const authResult = await checkAuth(request, env);
    ...
    try {
      switch (request.method) {
        ...
        case 'POST':
          ...
          } else if (path === '/test-bark') {
            return await testBarkNotification(request, env);
          }
          break;
  ...
  // 获取订阅索引列表
  async function getServerIndex(env) {
    try {
  ...
  // 保存订阅索引列表
  async function saveServerIndex(env, index) {
    try {
  ...
  // 保存单个订阅数据
  async function saveServerById(env, serverId, serverData) {
    try {
  ...
  // 删除单个订阅数据
  async function deleteServerById(env, serverId) {
    try {
  ...
  // 批量获取订阅数据
  async function getBatchServers(env, serverIds) {
    try {
      const promises = serverIds.map(id => getServerById(env, id));
  ...
  async function getServers(env) {
    try {
      const serverIds = await getServerIndex(env);
      const servers = await getBatchServers(env, serverIds);
  ...
  async function addServer(request, env) {
    try {
      ...
      if (existingServer) {
        return new Response(JSON.stringify({ 
          error: `订阅名称已存在，与"${existingServer.name}"冲突，请使用不同的名称`,
          code: 'DUPLICATE_NAME'
        }), {
          status: 400,
          headers: { 'Content-Type': 'application/json' }
      ...
      // 保存订阅数据到独立存储
      await saveServerById(env, server.id, server);
      
      // 更新索引列表
      serverIds.push(server.id);
      await saveServerIndex(env, serverIds);
  ...
  async function deleteServer(request, env) {
    try {
      ...
      // 从索引中移除该订阅ID
      const filteredServerIds = serverIds.filter(id => id !== serverId);
      await saveServerIndex(env, filteredServerIds);
      
      // 删除订阅数据
      await deleteServerById(env, serverId);
  ...
  async function updateServer(request, env) {
    try {
      ...
      const existingServer = await getServerById(env, serverId);
      if (!existingServer) {
      ...
      // 更新订阅信息
      const updatedServer = { ...existingServer, ...updatedServerData, updatedAt: new Date().toISOString() };
      
      // 保存到独立存储
      await saveServerById(env, serverId, updatedServer);
      
      return new Response(JSON.stringify({ success: true, server: updatedServer }), {
        headers: { 'Content-Type': 'application/json' }
      });
    } catch (error) {
  ...
  async function deleteCategory(request, env) {
    try {
      ...
      // 将该分类下的所有订阅移动到默认分类（设置categoryId为空字符串）
      ...
      // 批量更新受影响的订阅
      const affectedServers = servers.filter(server => server.categoryId === categoryId);
      const updatePromises = affectedServers.map(server => saveServerById(env, server.id, server));
  ...
  async function cleanupServers(request, env) {
    try {
      ...
      // 保存每个订阅到独立存储
      const savePromises = servers.map(server => saveServerById(env, server.id, server));
  ...
  async function getSettings(env) {
    try {
      ...
      const settings = {
        bark: {
          url: config.bark?.configSource?.hasExternal ? config.bark.url : (config.bark?.url || ''),
        ...
        auth: {
          ...
          twoFactorSecret: config.auth.configSource.hasExternal ? config.auth.twoFactorSecret : (config.auth.twoFactorSecret || '')
        },
      ...
      // 添加配置来源信息
      settings.bark.configSource = config.bark?.configSource;
  ...
  async function saveSettings(request, env) {
    try {
      ...
      const externalBarkConfig = hasExternalBarkConfig(env);
      ...
      // 验证Bark配置：如果没有外置配置且启用了Bark通知，则必须填写URL
      if (!externalBarkConfig.hasExternal && settings.bark && settings.bark.enabled) {
        if (!settings.bark.url || settings.bark.url.trim() === '') {
          return new Response(JSON.stringify({ error: '启用Bark推送后，必须填写Bark URL' }), {
            status: 400,
            headers: { 'Content-Type': 'application/json' }
          });
        }
  ...
  // Bark测试通知（服务端发送，避免浏览器跨域误报）
  async function testBarkNotification(request, env) {
    try {
      const body = await request.json();
      const config = await getFullConfig(env);
      const barkUrl = (body?.url || config.bark?.url || '').trim();
      const barkEnabled = !!(config.bark?.enabled || body?.url);

      if (!barkEnabled) {
        return new Response(JSON.stringify({ success: false, error: '请先启用Bark推送功能' }), {
          status: 400,
          headers: { 'Content-Type': 'application/json' }
        });
      }
      ...
      if (!barkUrl) {
        return new Response(JSON.stringify({ success: false, error: 'Bark URL不能为空' }), {
          status: 400,
          headers: { 'Content-Type': 'application/json' }
        });
      }

      const message = `🧪 订阅监控系统测试通知\n\n这是一条测试消息，用于验证Bark通知配置是否正确。\n\n发送时间：${new Date().toLocaleString('zh-CN')}`;
      await sendBarkMessageByUrl(barkUrl, 'Bark配置测试', message);

      return new Response(JSON.stringify({ success: true }), {
        headers: { 'Content-Type': 'application/json' }
      });
    } catch (error) {
      return new Response(JSON.stringify({ success: false, error: error.message || 'Bark测试通知发送失败' }), {
        status: 500,
        headers: { 'Content-Type': 'application/json' }
      });
    }
...
// ==========================================
  ...
  // 发送Bark消息
  async function sendBarkMessageByUrl(baseUrl, title, message) {
    const endpoint = `${baseUrl.replace(/\/$/, '')}?title=${encodeURIComponent(title)}&body=${encodeURIComponent(message)}`;
    const response = await fetch(endpoint, { method: 'GET' });
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(`Bark接口返回异常: ${response.status} - ${errorText}`);
    }
  }
  ...
  async function sendBatchTelegramNotification(expiredServers, warningServers, env) {
    try {
      ...
      const telegramEnabled = !!(enabled && botToken && chatId);
      const barkEnabled = !!(config.bark?.enabled && config.bark?.url);
      if (!telegramEnabled && !barkEnabled) return;

      let message = '🔔 订阅到期监控报告\n\n';
      message += `📅 检查时间: ${new Date().toLocaleString('zh-CN')}\n\n`;

      // 添加已过期订阅信息
      if (expiredServers.length > 0) {
        message += `❌ 已过期订阅 (${expiredServers.length}台):\n`;
        message += '━━━━━━━━━━━━━━━━\n';
      ...
      if (warningServers.length > 0) {
        message += `⚠️ 即将到期订阅 (${warningServers.length}台):\n`;
        message += '━━━━━━━━━━━━━━━━\n';
...
// ==========================================
  ...
  function getHTML(settings = {}) {
    // 获取自定义设置，如果没有则使用默认值
    const siteTitle = (settings.siteTitle && settings.siteTitle.trim() !== '') ? settings.siteTitle : '订阅到期监控';
  ...
  const require2FA = !!(settings.auth && settings.auth.twoFactorSecret);
  // 将require2FA注入到前端JavaScript中
  const require2FAScript = `<script>window.require2FA = ${require2FA};</script>`;
  return `<!DOCTYPE html>
  <html lang="zh-CN">
  <head>
      <meta charset="UTF-8">
  ...
  <body>
      ...
      <main class="main-content">
          ...
          <div class="overview-section">
              <div class="overview-left">
                  ...
                  <div class="stats-grid">
                      <div class="stat-card total" onclick="filterServers('all')" title="点击查看所有订阅">
                          <h3><i class="iconfont icon-circle-info"></i> 总订阅</h3>
                          <div class="value" id="totalServers">
                              <span class="status-indicator online"></span>
                              0
                          </div>
                      </div>
                      <div class="stat-card online" onclick="filterServers('online')" title="点击只查看正常运行的订阅">
                      ...
                      <div class="stat-card warning" onclick="filterServers('warning')" title="点击只查看即将过期的订阅">
                      ...
                      <div class="stat-card offline" onclick="filterServers('offline')" title="点击只查看已过期的订阅">
          ...
          <div class="servers-section">
              <div class="section-header">
                  <div class="section-title"><i class="iconfont icon-list-ul"></i> 订阅列表</div>
                  <div class="section-actions">
                      ...
                      <button class="btn btn-primary" onclick="showAddServerModal('')"><i class="iconfont icon-jia1"></i> 添加订阅</button>
          ...
          <div class="modal-content">
              <div class="modal-header">
                  <div class="modal-title-section">
                      <div class="modal-title">添加新订阅</div>
                      <button class="import-btn-header" id="importFromClipboardBtn" title="从剪贴板导入订阅信息">
              ...
              <form id="addServerForm">
                  <div class="form-grid">
                      <div class="form-group full-width">
                          <label for="serverName"><i class="iconfont icon-hollow-computer"></i> 订阅名称<span class="required">*</span></label>
                      ...
                      <div class="form-group">
                          <label for="serverIP"><i class="iconfont icon-earth-full"></i> 备注<span class="required">*</span></label>
                      ...
                      <form id="settingsForm">
                          ...
                          <div class="settings-tab active" id="basicTab">
                              ...
                              <div class="form-group" style="margin-bottom: 24px;">
                                  ...
                                  <input type="text" id="siteTitle" placeholder="订阅到期监控" maxlength="50">
                                  <div class="form-help">
                                      显示在页面顶部导航栏的标题文字
                                  </div>
                              </div>
                          ...
                          <div class="settings-tab" id="notificationTab">
                              <h3 class="settings-tab-title"><i class="iconfont icon-bell"></i> Bark推送设置</h3>
                              
                              <div class="form-group" style="margin-bottom: 24px;">
                                  <div class="checkbox-wrapper">
                                      <input type="checkbox" id="enableBarkNotification" onchange="toggleBarkConfig()">
                                      <label for="enableBarkNotification">启用Bark推送</label>
                                  </div>
                                  <div class="form-help">
                                      启用后可以通过Bark接收订阅到期提醒
                                  </div>
                              ...
                              <!-- 外置Bark配置提示 -->
                              <div id="externalBarkNotice" class="form-notice" style="display: none; background-color: #e8f5e8; border-left: 4px solid #4CAF50; color: #2d5a2d;">
                                  <i class="iconfont icon-check-circle" style="color: #4CAF50; margin-right: 8px;"></i>
                                  <span id="externalBarkText">已在外置环境变量中配置Bark，推送功能已自动启用。</span>
                              </div>

                              <div id="barkConfigSection">
                                  <div class="form-group" style="margin-bottom: 24px;">
                                      <label for="barkUrl"><i class="iconfont icon-earth-full"></i> Bark URL</label>
                                      <div class="password-input-wrapper">
                                          <input type="text" id="barkUrl" placeholder="例如: https://api.day.app/你的Key/">
                                      </div>
                                      <div class="form-help" id="barkUrlHelp">
                                          推送服务URL，支持自建Bark服务端
                                      </div>
                                  </div>
                                  <div class="form-group" style="align-items: flex-start; margin-bottom: 40px;">
                                      <button type="button" class="btn btn-primary" onclick="testBarkNotification()" id="testBarkBtn">
                                          <i class="iconfont icon-paper-plane"></i> Bark测试
                                      </button>
                                  </div>
                              </div>
                          ...
                          <div class="settings-tab" id="securityTab">
                              ...
                              <!-- 2FA 设置部分 -->
                              <div class="form-group" style="margin-bottom: 24px;">
                                  <label for="twoFactorSecret"><i class="iconfont icon-anquan"></i> Google 2FA 密钥</label>
                                  <div class="password-input-wrapper" style="display:flex; gap: 8px; align-items: stretch;">
                                      <input type="text" id="twoFactorSecret" placeholder="留空则不启用2FA，填写Base32格式密钥" maxlength="64" style="flex:1; border-radius: 4px; border: 1px solid var(--border-color); padding: 8px 12px; transition: border-color 0.3s; outline: none; background: var(--bg-primary); color: var(--text-primary); font-size: 14px;">
                                      <button type="button" class="btn" style="padding: 0 16px; background-color: #f1f5f9; border: 1px solid #e2e8f0; color: #475569; border-radius: 4px; cursor: pointer; transition: all 0.3s;" onclick="generate2FA()" id="btnGen2FA">生成密钥</button>
                                  </div>
                                  <div class="form-help">
                                      填入由字母或数字组成的Base32密钥。如果配置将作为登录时的强制两步验证。
                                  </div>
                                  <div id="qrCodeContainer" style="display:none; margin-top:16px; text-align:center;">
                                      <p style="margin-bottom: 8px; color: var(--text-secondary); font-size: 13px;">请使用 Google Authenticator 或其他支持 TOTP 的应用扫码：</p>
                                      <img id="qrCodeImage" src="" alt="2FA QR Code" style="width: 150px; height: 150px; border: 1px solid var(--border-color); border-radius: 8px; padding: 8px; background: white;">
                                  </div>
                              </div>
                              <div id="external2FANotice" class="form-notice" style="display: none; background-color: #e8f5e8; border-left: 4px solid #4CAF50; color: #2d5a2d; margin-bottom: 30px;">
                                  <i class="iconfont icon-check-circle" style="color: #4CAF50; margin-right: 8px;"></i>
                                  <span>已在外置环境变量中配置2FA，两步验证已自动启用。</span>
                              </div>
      ...
      <div class="modal" id="editServerModal">
          <div class="modal-content">
              <div class="modal-header">
                  <div class="modal-title">编辑订阅信息</div>
                  <button class="close-btn" onclick="hideEditServerModal()" title="关闭 (ESC)">&times;</button>
              </div>
              <form id="editServerForm">
                  <div class="form-grid">
                      <div class="form-group full-width">
                          <label for="editServerName"><i class="iconfont icon-hollow-computer"></i> 订阅名称<span class="required">*</span></label>
                      ...
                      <div class="form-group">
                          <label for="editServerIP"><i class="iconfont icon-earth-full"></i> 备注<span class="required">*</span></label>
      ...
      <div class="modal" id="renewalModal">
          <div class="modal-content">
              <div class="modal-header">
                  <div class="modal-title">订阅续期</div>
      ...
      <footer class="footer">
          <div class="footer-content">
              <span class="footer-text">Copyright © 2026 Netsky</span>
              <span class="footer-divider">|</span>
              <a href="https://github.com/cnfug/CF-Server-AutoCheck" class="footer-link" target="_blank">
                  <i class="iconfont icon-github"></i>
                  GitHub Repository
              </a>
              <span class="footer-divider">|</span>
              <a href="https://nas.yt" class="footer-link">
      ...
      <script>
          ...
          function sortServers(serversArray) {
              ...
              sortedServers.sort((a, b) => {
                  ...
                  switch (currentSortField) {
                      case 'addTime':
                          // 使用订阅ID作为添加时间的代理（假设ID是递增的）
                          aValue = a.id || 0;
                          bValue = b.id || 0;
                          break;
                      case 'name':
                          // 订阅名称：移除emoji，应用拼音转换，转小写（名称必填，无需处理空值）
          ...
          async function loadData() {
              try {
                  ...
                  await cleanupOrphanedServers(); // 清理孤儿订阅
                  await loadStats();
          ...
          function toggleBarkConfig() {
              const checkbox = document.getElementById('enableBarkNotification');
              const section = document.getElementById('barkConfigSection');
              const input = document.getElementById('barkUrl');
              const testBtn = document.getElementById('testBarkBtn');
              
              if (hasExternalBarkConfig) return;
          ...
          function show2FAQRCode(secret) {
              const qrCodeContainer = document.getElementById('qrCodeContainer');
              if (!secret) {
                  qrCodeContainer.style.display = 'none';
                  return;
              }
              const siteTitle = document.getElementById('siteTitle').value || '订阅监控系统';
              const issuer = '订阅监控';
              const uri = \`otpauth://totp/\${encodeURIComponent(issuer)}:\${encodeURIComponent(siteTitle)}?secret=\${secret}&issuer=\${encodeURIComponent(issuer)}\`;
          ...
          async function loadSettings() {
              try {
                  ...
                  const settings = await response.json();
                            ...
                            // 填充表单数据
          document.getElementById('enableBarkNotification').checked = settings.bark?.enabled || false;
                  document.getElementById('barkUrl').value = settings.bark?.url || '';

                  const enableBarkCheckbox = document.getElementById('enableBarkNotification');
                  const barkNotice = document.getElementById('externalBarkNotice');
                  const barkNoticeText = document.getElementById('externalBarkText');
                  
                  if (settings.bark?.configSource && settings.bark.configSource.hasExternal) {
                      hasExternalBarkConfig = true;
                      enableBarkCheckbox.disabled = true;
                      barkNotice.style.display = 'flex';
                      if (settings.bark.configSource.source === 'environment') {
                          barkNoticeText.textContent = '已在环境变量中配置Bark参数，不可关闭。';
                      } else {
                          barkNoticeText.textContent = '已在代码中配置Bark参数，不可关闭。';
                      }
                      document.getElementById('barkUrl').disabled = true;
                      if (!settings.bark?.url) document.getElementById('barkUrl').value = '已通过外部配置设置';
                  } else {
                      hasExternalBarkConfig = false;
                      enableBarkCheckbox.disabled = false;
                      barkNotice.style.display = 'none';
                      document.getElementById('barkUrl').disabled = false;
                  }

                  document.getElementById('twoFactorSecret').value = settings.auth?.twoFactorSecret || '';
                  show2FAQRCode(settings.auth?.twoFactorSecret || '');
                  const twoFactorNotice = document.getElementById('external2FANotice');
                  const twoFactorInput = document.getElementById('twoFactorSecret');
                  const gen2FAButton = document.getElementById('btnGen2FA');
                  if (settings.auth?.configSource && settings.auth.configSource.hasExternal && settings.auth?.twoFactorSecret) {
                      twoFactorNotice.style.display = 'flex';
                      twoFactorInput.disabled = true;
                      if (gen2FAButton) gen2FAButton.disabled = true;
                  } else {
                      twoFactorNotice.style.display = 'none';
                      twoFactorInput.disabled = false;
                      if (gen2FAButton) gen2FAButton.disabled = false;
                  }
          ...
          document.getElementById('siteTitle').value = (settings.siteTitle && settings.siteTitle !== '订阅到期监控') ? settings.siteTitle : '';
          ...
          async function testBarkNotification() {
              const enableCheckbox = document.getElementById('enableBarkNotification');
              const hasExternalConfig = enableCheckbox.disabled;
              
              if (!enableCheckbox.checked) {
                  showNotification('请先启用Bark推送功能', 'warning');
                  return;
              }
              ...
              if (!barkUrl) {
                  showNotification('Bark配置不完整，无法发送测试通知', 'warning');
                  return;
              }
              
              const testBtn = document.getElementById('testBarkBtn');
              const originalHTML = testBtn.innerHTML;
              testBtn.innerHTML = '<i class="iconfont icon-loading" style="display:inline-block;animation:spin 1s linear infinite;"></i> 发送中...';
              ...
              try {
                  const response = await fetch('/api/test-bark', {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({ url: barkUrl })
                  });

                  const result = await response.json();
                  if (response.ok && result.success) {
                      showNotification('Bark测试通知发送成功！请检查您的设备。', 'success');
                  } else {
                      throw new Error(result.error || '发送失败');
                  }
              } catch (error) {
                  showNotification('Bark测试通知发送失败：' + (error.message || '网络连接错误'), 'error');
              } finally {
                      ...
                      <div class="category-section">
                          <div class="category-header">
                              ...
                              <div class="category-actions">
                                  <button class="action-btn danger" onclick="batchDeleteServers('')" title="批量删除选中的订阅" id="batchDeleteBtn-" style="display: none;"><i class="iconfont icon-shanchu"></i> 删除选中 (<span id="selectedCount-">0</span>)</button>
                                  <button class="action-btn primary" onclick="showAddServerModal('')" title="添加订阅到默认分类"><i class="iconfont icon-jia1"></i> 添加</button>
                              </div>
                          </div>
                          \${defaultContent}
                      </div>
                  ...
                  html += \`
                      <div class="category-section">
                          <div class="category-header">
                              ...
                              <div class="category-actions">
                                  <button class="action-btn danger" onclick="batchDeleteServers('\${category.id}')" title="批量删除选中的订阅" id="batchDeleteBtn-\${category.id}" style="display: none;"><i class="iconfont icon-shanchu"></i> 删除选中 (<span id="selectedCount-\${category.id}">0</span>)</button>
                                  <button class="action-btn primary" onclick="showAddServerModal('\${category.id}')" title="添加订阅到此分类"><i class="iconfont icon-jia1"></i> 添加</button>
                      ...
                      <div class="monitor-card-header">
                          <div class="monitor-title-section">
                              ...
                              <div class="server-name-container">
                                  <h3 class="monitor-vps-title" onclick="editServer('\${server.id}')" title="点击编辑订阅信息" style="font-weight: bold;">
                                  \${server.name}
                                  </h3>
                                  <i class="iconfont icon-ic_line_copy24px server-name-copy-btn" onclick="copyServerInfo('\${server.id}')" title="复制订阅信息"></i>
                              </div>
                          </div>
                      </div>
                      ...
                      <div class="monitor-card-footer">
                          <div class="monitor-team-section">
                              <div class="ip-label-container">
                                  <span class="monitor-team-label">备注</span>
                                  \${server.ip && server.ip !== '未设置' ? \`<i class="iconfont icon-ic_line_copy24px ip-copy-btn" onclick="copyIPAddress('\${server.ip}')" title="复制备注"></i>\` : ''}
                              </div>
          ...
          function setupForms() {
                  ...
                  try {
                      ...
                      if (response.ok) {
                          ...
                          if (!trimmedNewName) {
                              ...
                              showNotification('请输入订阅名称', 'warning');
                              return false;
                          }
                          ...
                          if (conflictServer) {
                              nameInput.classList.add('input-error');
                              showNotification(\`订阅名称已存在，与"\${conflictServer.name}"冲突，请使用不同的名称\`, 'warning');
              ...
              async function checkEditServerNameDuplicate(name, currentServerId) {
                  try {
                      ...
                      if (response.ok) {
                          ...
                          // 查找冲突的订阅，但排除当前正在编辑的订阅
                          const conflictServer = servers.find(server => {
                              if (server.id === currentServerId) return false; // 排除当前编辑的订阅
                              return server.name.trim() === trimmedNewName;
                          });
                      ...
                      console.error('检查编辑订阅名称失败:', error);
                      return false;
                  }
              }
              ...
              async function importFromClipboard() {
                  try {
                      ...
                      if (navigator.clipboard && window.isSecureContext) {
                          ...
                          // 降级方案：提示用户手动粘贴
                          clipboardText = prompt('请粘贴订阅信息数据：');
                          if (!clipboardText) {
                              return;
                          }
                      }
                      ...
                      try {
                          ...
                          showNotification('剪贴板数据格式错误，请确保是有效的订阅信息', 'error');
                          return;
                      }
                      
                      // 验证必要字段
                      if (!serverData || typeof serverData !== 'object') {
                          showNotification('无效的订阅数据', 'error');
                          return;
                      }
                      ...
                      showNotification('订阅信息已成功导入到表单', 'success');
                      
                      // 如果粘贴了订阅名称，检查是否存在同名订阅
                      if (serverData.name) {
                          // 延迟一点执行，确保导入成功消息先显示
                          setTimeout(async () => {
                  ...
                  try {
                      ...
                      if (response.ok) {
                          ...
                          if (errorData.code === 'DUPLICATE_NAME' || errorData.code === 'EMPTY_NAME') {
                              ...
                              throw new Error(errorData.error || '添加订阅失败');
                          }
                      }
                  } catch (error) {
              ...
              document.getElementById('settingsForm').addEventListener('submit', async function(e) {
                  ...
                  const formData = {
                      bark: {
                          enabled: document.getElementById('enableBarkNotification').checked,
                      ...
                      auth: {
                          ...
                          twoFactorSecret: document.getElementById('twoFactorSecret').value.trim()
                      },
                  ...
                  try {
                      ...
                      if (response.ok) {
                          ...
                          if (authAction === 'reauth') {
                              ...
                              // 延迟跳转，让用户看到成功提示
                              setTimeout(async () => {
                                  // HttpOnly Cookie无法通过JavaScript删除，需要调用订阅API
                                  try {
                                      // 调用登出API清除订阅端的Cookie
              ...
              document.getElementById('editServerForm').addEventListener('submit', async function(e) {
                  ...
                  if (!currentEditServerId) {
                      showNotification('订阅ID未找到', 'error');
                      return;
                  }
                  ...
                  try {
                      ...
                      if (response.ok) {
                          ...
                          showNotification('订阅信息更新成功！', 'success');
                          hideEditServerModal();
                          await loadData();
                      } else {
                          const errorData = await response.json();
                          throw new Error(errorData.error || '更新订阅失败');
                      }
                  } catch (error) {
                      showNotification('错误：' + error.message, 'error');
                  }
          ...
          async function updatePageDisplay(settings) {
              ...
              const siteTitle = settings.siteTitle || '订阅到期监控';
              ...
              // 更新页面title
              document.title = siteTitle + ' - 订阅监控面板';
          ...
          async function deleteServer(serverId) {
              ...
              const serverName = server ? server.name : '未知订阅';
              
              const confirmed = await showConfirmDialog(
                  '删除订阅',
                  \`您确定要删除订阅 "\${serverName}" 吗？\\n\\n此操作不可恢复。\`,
          ...
          async function deleteCategory(categoryId) {
              ...
              try {
                  ...
                  if (response.ok) {
                      // 完全重新加载所有数据
                      await loadData();
                      showNotification('分类删除成功！该分类下的订阅已移动到默认分类。', 'success');
                  } else {
                      const errorData = await response.json();
          ...
          function openRenewalLink(serverId) {
              ...
              if (!server || !server.renewalLink) {
                  showNotification('该订阅没有设置续期链接', 'warning');
                  return;
              }
          ...
          async function copyIPAddress(ip) {
              try {
                  ...
                  showNotification(\`备注 \${ip} 已复制到剪贴板\`, 'success');
              } catch (error) {
          ...
          async function copyServerInfo(serverId) {
              try {
                  ...
                  if (!server) {
                      showNotification('订阅信息未找到', 'error');
                      return;
                  }
                  ...
                  const serverTemplate = {
                      ...
                      ip: server.ip || '',
                      // categoryId: 不复制分类信息，让用户在当前分类下添加订阅
                      tags: server.tags || '',
                  ...
                  showNotification(\`订阅信息 "\${server.name}" 已复制到剪贴板\`, 'success');
              } catch (error) {
                  console.error('复制失败:', error);
                  showNotification('复制失败，请手动复制', 'error');
              }
          ...
          function showRenewalModal(serverId) {
              ...
              // 智能选择续期起始日期类型
              // 如果订阅有保存的续期偏好，使用保存的偏好（但排除"自定义"和已废弃的"从当前日期累计"）
              // 否则默认选择"从到期日期开始"
              ...
              try {
                ...
                const updatedServer = {
                  ...
                  if (response.ok) {
                      await loadData();
                      hideRenewalModal();
                      showNotification(\`订阅续期成功！新的到期日期：\${newExpireDate}\`, 'success');
                  } else {
          ...
          let originalServerData = null; // 保存原始订阅数据
          
          // 清除上次续期日期（仅在界面上标记，不立即保存）
          async function clearLastRenewalDate() {
              if (!currentEditServerId) {
                  showNotification('未找到订阅信息', 'error');
                  return;
              }
          ...
          async function batchDeleteServers(categoryId) {
              ...
              const confirmed = await showConfirmDialog(
                  '批量删除订阅',
                  \`您确定要删除 "\${categoryName}" 分类下的以下 \${categoryServers.length} 台订阅吗？\\n\\n\${serverNames}\\n\\n此操作不可恢复。\`,
              ...
              try {
                  ...
                  if (successCount > 0) {
                      ...
                      if (errorCount === 0) {
                          showNotification(\`成功删除 \${successCount} 台订阅！\`, 'success');
                      } else {
                          showNotification(\`成功删除 \${successCount} 台订阅，\${errorCount} 台删除失败\`, 'warning');
                      }
                  } else {
...
function getLoginHTML(settings = {}) {
  const require2FA = !!(settings.auth && settings.auth.twoFactorSecret);
  const siteTitle = (settings.siteTitle && settings.siteTitle.trim() !== '') ? settings.siteTitle : '订阅监控系统';
  // 获取自定义Logo URL，如果没有则使用默认值
  const logoUrl = (settings.customLogoUrl && settings.customLogoUrl.trim() !== '') ? settings.customLogoUrl : LOGO_IMAGE_URL;
  // 根据Logo格式确定CSS类
  const logoClass = logoUrl.toLowerCase().includes('.svg') || logoUrl.toLowerCase().includes('format=svg') ? 'logo-image svg-logo' : 'logo-image raster-logo';
...
<body>
    <!-- GitHub角标 -->
    <a href="https://github.com/cnfug/CF-Server-AutoCheck" target="_blank" class="github-corner" title="GitHub Repository">
    ...
    <div class="login-container">
        ...
        <form class="login-form" id="loginForm">
            ...
            <div class="form-group">
                <label for="twoFactorCode">
                    <i class="iconfont icon-anquan"></i> 两步验证码
                </label>
                <input 
                    type="text" 
                    id="twoFactorCode" 
        ...
        <div class="footer">
            <p>Copyright &copy; 2026 Netsky</p>
        </div>
    </div>
        ...
        document.getElementById('loginForm').addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('/login', {
                    ...
                    body: JSON.stringify({ password: password, twoFactorCode: window.require2FA ? document.getElementById('twoFactorCode').value : undefined })
                });
...
async function handleLogin(request, env) {
    try {
        ...
        if (config.auth.twoFactorSecret) {
            if (!twoFactorCode) {
                return new Response(JSON.stringify({ 
                    success: false, 
                    error: '请输入两步验证码' 
            ...
            const isValid2FA = await verifyTOTP(config.auth.twoFactorSecret, twoFactorCode);
            if (!isValid2FA) {
                return new Response(JSON.stringify({ 
                    success: false, 
                    error: '验证码错误或已过期' 
...
async function getConfigValue(env, category, key, webSettings = null) {
    ...
    const envVarMap = {
        ...
        'auth.password': 'PASS',
        'auth.twoFactorSecret': 'GA_SECRET',
        'bark.url': 'BARK_URL'
    };
    ...
    if (category === 'telegram') {
        ...
        if (key === 'url') codeValue = BARK_URL;
    } else if (category === 'auth') {
        if (key === 'password') codeValue = AUTH_PASSWORD;
        else if (key === 'twoFactorSecret') codeValue = GOOGLE_AUTH_SECRET;
    }
...
function hasExternalBarkConfig(env) {
    const hasEnvConfig = !!env.BARK_URL;
    const hasCodeConfig = BARK_URL && BARK_URL.trim() !== '';
    return {
...
function hasExternalAuthConfig(env) {
    // 检查环境变量
    const hasEnvConfig = (env.PASS && env.PASS.trim() !== '') || (env.GA_SECRET && env.GA_SECRET.trim() !== '');
    
    // 检查代码配置
    const hasCodeConfig = (AUTH_PASSWORD && AUTH_PASSWORD.trim() !== '') || (GOOGLE_AUTH_SECRET && GOOGLE_AUTH_SECRET.trim() !== '');
...
async function syncConfigurationState(env, forceCleanup = false) {
    try {
        ...
        // 确保设置对象结构完整
        if (!updatedSettings.bark) {
            updatedSettings.bark = { enabled: false, url: '' };
        }
...
async function getFullConfig(env, forceSync = false) {
    ...
    return {
        bark: {
            enabled: hasExternalBarkConfig(env).hasExternal || (webSettings.bark?.enabled || false),
            url: await getConfigValue(env, 'bark', 'url', webSettings),
            configSource: hasExternalBarkConfig(env)
        },
        telegram: {
        ...
        auth: {
            ...
            twoFactorSecret: await getConfigValue(env, 'auth', 'twoFactorSecret', webSettings),
            configSource: externalAuthConfig
        },
        globalNotifyDays: webSettings.globalNotifyDays || 14,
        siteTitle: (webSettings.siteTitle && webSettings.siteTitle.trim() !== '') ? webSettings.siteTitle : '订阅到期监控',
...
<body>
    <div class="setup-container">
        <div class="setup-header">
            <img src="${LOGO_IMAGE_URL}" alt="订阅 Monitor">
            <h1>欢迎使用订阅到期监控系统</h1>
