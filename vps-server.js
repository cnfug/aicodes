...
// Bark通知配置
const BARK_SERVER = ""                  // 在此填写Bark服务器地址，或留空使用其他方式配置，可选择外置变量BARK_SERVER
const BARK_KEY = "";                   // 在此填写Bark设备Key，或留空使用其他方式配置，可选择外置变量BARK_KEY
const BARK_GROUP = "服务器监控";         // Bark通知分组名称
...
export default {
    async fetch(request, env, ctx) {
      ...
      if (url.pathname.startsWith('/api/')) {
        // 对于API请求，如果需要认证，则检查认证状态
        if (authRequired) {
          const authResult = await checkAuth(request, env);
          if (!authResult.isAuthenticated) {
            return new Response(JSON.stringify({ 你没有钥匙: '傻逼快滚' }), { 
              status: 401,
...
// ==========================================
    ...
    try {
      switch (request.method) {
        ...
        case 'POST':
              ...
              } else if (path === '/test-bark-notification') {
                const server = await request.json();
                await sendBarkNotification(server, 7, 'warning', env);
  ...
  async function getSettings(env) {
    try {
      ...
      const settings = {
        ...
        bark: {
          server: config.bark.configSource.hasExternal ? config.bark.server : (config.bark.enabled ? config.bark.server : ''),
          key: config.bark.configSource.hasExternal ? config.bark.key : (config.bark.enabled ? config.bark.key : ''),
  ...
  async function saveSettings(request, env) {
    try {
      ...
      if (externalBarkConfig.hasExternal) {
        settings.bark.enabled = true;
      }
      
      // 确保Bark设置对象存在
      if (!settings.bark) {
        settings.bark = { enabled: false, server: '', key: '' };
      }
      ...
      // 验证Bark配置：如果没有外置配置且启用了Bark通知，则必须填写Key
      if (!externalBarkConfig.hasExternal && settings.bark.enabled) {
        if (!settings.bark.key) {
          return new Response(JSON.stringify({ error: '启用Bark通知后，必须设置Bark设备Key' }), {
            status: 400,
            headers: { 'Content-Type': 'application/json' }
          });
        }
...
// ==========================================
  ...
  async function checkAndNotifyExpiredVPS(env) {
    try {
      ...
      if (expiredServers.length > 0 || warningServers.length > 0) {
        ...
        // 发送Bark通知
        await sendBatchBarkNotification(expiredServers, warningServers, env);
      }
    } catch (error) {
  ...
  // 发送单个服务器Bark通知
  async function sendBarkNotification(server, daysLeft, type, env) {
    try {
      const config = await getFullConfig(env);
      const { server: barkServer, key: barkKey, enabled, group } = config.bark;
      
      // 检查是否启用Bark通知且配置完整（必须同时有barkServer和barkKey）
      if (!enabled || !barkKey || !barkServer) return;
      
      // 只使用用户配置的服务器地址
      const baseUrl = barkServer.replace(/\/$/, ''); // 移除末尾斜杠
      ...
      // 构建Bark API URL
      const barkUrl = `${baseUrl}/${barkKey}/${encodeURIComponent(title)}/${encodeURIComponent(body)}?group=${encodeURIComponent(group)}&sound=alarm`;
  ...
  // 发送批量Bark通知
  async function sendBatchBarkNotification(expiredServers, warningServers, env) {
    try {
      const config = await getFullConfig(env);
      const { server: barkServer, key: barkKey, enabled, group } = config.bark;
      ...
      // 构建与Telegram完全一致的消息内容
      let body = `📅 检查时间: ${new Date().toLocaleString('zh-CN')}\n\n`;

      // 添加已过期服务器信息
      if (expiredServers.length > 0) {
        body += `❌ 已过期服务器 (${expiredServers.length}台):\n`;
        body += '━━━━━━━━━━━━━━━━\n';
        
        expiredServers.forEach(({ server, daysLeft }) => {
          body += `🔸 ${server.name}\n`;
          body += `   已过期: ${Math.abs(daysLeft)} 天\n`;
          body += `   到期日期: ${server.expireDate}\n`;
          body += `   IP地址: ${server.ip || '未知'}\n`;
          body += `   服务商: ${server.provider || '未知'}\n`;
          if (server.renewalLink && server.renewalLink.trim() !== '') {
            body += `   续期链接: ${server.renewalLink}\n`;
          } else {
            body += `   续期链接: 未设置\n`;
          }
          body += `\n`;
      ...
      // 添加即将到期服务器信息
      if (warningServers.length > 0) {
        body += `⚠️ 即将到期服务器 (${warningServers.length}台):\n`;
        body += '━━━━━━━━━━━━━━━━\n';
        
        warningServers.forEach(({ server, daysLeft }) => {
          body += `🔸 ${server.name}\n`;
          body += `   剩余天数: ${daysLeft} 天\n`;
          body += `   到期日期: ${server.expireDate}\n`;
          body += `   IP地址: ${server.ip || '未知'}\n`;
          body += `   服务商: ${server.provider || '未知'}\n`;
          if (server.renewalLink && server.renewalLink.trim() !== '') {
      ...
      if (!response.ok) {
        ...
        console.error('Bark notification failed:', response.status, errorText);
        return;
      }
      
      const responseData = await response.json();
      console.log('Bark notification sent successfully:', responseData);
    } catch (error) {
      console.error('Send batch bark notification error:', error);
    }
  }
...
// ==========================================
  ...
  <body>
                  ...
                  <div class="settings-content">
                      <form id="settingsForm">
                          ...
                          <div class="settings-tab" id="notificationTab">
                              ...
                              <h3 class="settings-tab-title" style="margin-top: 30px;"><i class="iconfont icon-lingdang"></i> Bark通知设置</h3>
                                
                              <div class="form-group" style="margin-bottom: 24px;">
                                <div class="checkbox-wrapper">
                                  <input type="checkbox" id="enableBarkNotification" onchange="toggleBarkConfig()">
                                  <label for="enableBarkNotification">启用Bark通知</label>
                                </div>
                                <div class="form-help">
                                  启用后可以接收服务器到期提醒和测试通知（适用于iOS设备）
                                </div>
                              </div>
                                
                              <!-- Bark外置配置提示 -->
                              <div id="externalBarkConfigNotice" class="form-notice" style="display: none; background-color: #e8f5e8; border-left: 4px solid #4CAF50; color: #2d5a2d;">
                                <i class="iconfont icon-check-circle" style="color: #4CAF50; margin-right: 8px;"></i>
                                <span id="externalBarkConfigText">已在外置环境变量中配置Bark参数，通知功能已自动启用且不可关闭。</span>
                              </div>
                                
                              <div id="barkConfigSection">
                                <div class="form-group" style="margin-bottom: 24px;">
                                  <label for="barkServer"><i class="iconfont icon-link"></i> 服务器地址（可选）</label>
                                  <input type="url" id="barkServer" placeholder="https://api.day.app（默认）">
                                  <div class="form-help">
                                    自定义Bark服务器地址，留空使用官方默认服务器
                                  </div>
                                </div>
                                  
                                <div class="form-group" style="margin-bottom: 24px;">
                                  <label for="barkKey"><i class="iconfont icon-key"></i> 设备Key <span class="required">*</span></label>
                                  <div class="password-input-wrapper">
                                    <input type="password" id="barkKey" placeholder="请输入Bark设备Key">
                                    <i class="iconfont icon-bukejian password-toggle" onclick="togglePasswordVisibility('barkKey')" title="显示/隐藏Key"></i>
                                  </div>
                                  <div class="form-help">
                                    Bark应用中显示的设备Key，用于接收通知
                                  </div>
                                </div>
      ...
      <script>
          ...
          // 设置Bark配置
          if (settings.bark) {
            const barkEnabledCheckbox = document.getElementById('enableBarkNotification');
            const barkServerInput = document.getElementById('barkServer');
            const barkKeyInput = document.getElementById('barkKey');
            const externalBarkConfigNotice = document.getElementById('externalBarkConfigNotice');
            
            if (barkEnabledCheckbox) {
              barkEnabledCheckbox.checked = settings.bark.enabled || false;
            }
            
            if (barkServerInput) {
              barkServerInput.value = settings.bark.server || '';
            }
            
            if (barkKeyInput) {
              barkKeyInput.value = settings.bark.key || '';
            }
            
            if (externalBarkConfigNotice && settings.bark.configSource && settings.bark.configSource.hasExternal) {
              externalBarkConfigNotice.style.display = 'block';
              barkEnabledCheckbox.disabled = true;
              barkServerInput.disabled = true;
              barkKeyInput.disabled = true;
            }
          ...
          async function testBarkNotification() {
              ...
              try {
                  testBtn.innerHTML = '<i class="iconfont icon-loading"></i> 发送中...';
                  testBtn.disabled = true;
                  ...
                  const testServer = {
                      ...
                      ip: '192.168.1.1',
                      provider: '测试提供商',
                      expireDate: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0] // 7天后
                  };
                  
                  // 发送测试通知
                  const response = await fetch('/api/test-bark-notification', {
                      method: 'POST',
                      headers: {
                          'Content-Type': 'application/json'
                      },
                  ...
                  if (result.success) {
                      // 修复参数顺序：将标题和内容合并为一条消息，然后指定类型
                      showNotification('Bark测试通知已发送，请检查设备是否收到', 'success');
                  } else {
                      // 修复参数顺序：将标题和内容合并为一条消息，然后指定类型
                      showNotification('发送测试通知失败: ' + (result.error || '未知错误'), 'error');
                  }
              } catch (error) {
                  // 修复参数顺序：将标题和内容合并为一条消息，然后指定类型
                  showNotification('发送请求时发生错误', 'error');
              } finally {
          ...
          function setupForms() {
              ...
              document.getElementById('settingsForm').addEventListener('submit', async function(e) {
                  ...
                  telegram: {
                      enabled: document.getElementById('enableTelegramNotification').checked,
                      botToken: document.getElementById('telegramBotToken').value.trim(),
                      chatId: document.getElementById('telegramChatId').value.trim()
                  },
                  bark: {
                      enabled: document.getElementById('enableBarkNotification').checked,
                      server: document.getElementById('barkServer').value.trim(),
                      key: document.getElementById('barkKey').value.trim()
                  },
                  auth: {
  ...
  function toggleBarkConfig() {
      ...
      const configSection = document.getElementById('barkConfigSection');
      const barkKeyInput = document.getElementById('barkKey');
      const barkServerInput = document.getElementById('barkServer');
      ...
      if (barkKeyInput) {
          barkKeyInput.disabled = !isEnabled;
      }
      
      if (barkServerInput) {
          barkServerInput.disabled = !isEnabled;
      }
      
      // 如果禁用，清空输入框
      if (!isEnabled) {
          if (barkKeyInput) barkKeyInput.value = '';
          if (barkServerInput) barkServerInput.value = '';
      }
  }
...
async function getConfigValue(env, category, key, webSettings = null) {
    ...
    if (category === 'telegram') {
        ...
        if (key === 'server') codeValue = BARK_SERVER;
        else if (key === 'key') codeValue = BARK_KEY;
    }
...
// 检查是否存在外置Bark配置（环境变量或代码配置）
function hasExternalBarkConfig(env) {
    // 检查环境变量，确保值不为空字符串
    const hasEnvConfig = env.BARK_KEY && env.BARK_KEY.trim() !== '';
    
    // 检查代码配置，确保值不为空字符串
    const hasCodeConfig = BARK_KEY && BARK_KEY.trim() !== '';
    
    return {
        hasExternal: hasEnvConfig || hasCodeConfig,
        source: hasEnvConfig ? 'environment' : (hasCodeConfig ? 'code' : 'none'),
        envConfig: hasEnvConfig ? env.BARK_KEY : null,
        codeConfig: hasCodeConfig ? BARK_KEY : null
    };
}
...
async function checkAndStoreExternalConfigState(env) {
    try {
        ...
        const currentState = {
            ...
            bark: {
                hasExternal: externalBarkConfig.hasExternal,
                source: externalBarkConfig.source
            },
            lastCheck: Date.now()
        };
        ...
        if (lastState) {
            ...
            // Bark配置从有变为无
            if (lastState.bark?.hasExternal && !currentState.bark.hasExternal) {
                needsCleanup = true;
            }
        }
...
async function syncConfigurationState(env, forceCleanup = false) {
    try {
        ...
        if (!updatedSettings.bark) {
            updatedSettings.bark = { enabled: false, server: '', key: '' };
        }
        ...
        if (forceCleanup) {
            ...
            if (lastState) {
                ...
                // Bark配置：如果之前有外置配置，现在没有了，则清理
                if (lastState.bark?.hasExternal && !externalBarkConfig.hasExternal) {
                    updatedSettings.bark = {
                        enabled: false,
                        server: '',
                        key: ''
                    };
...
async function getFullConfig(env, forceSync = false) {
    ...
    return {
        ...
        bark: {
            // 修改为尊重网页设置，只有在网页端未明确禁用时才使用外置配置
            enabled: (externalBarkConfig.hasExternal && webSettings.bark?.enabled !== false) || (webSettings.bark?.enabled === true),
            server: await getConfigValue(env, 'bark', 'server', webSettings),
            key: await getConfigValue(env, 'bark', 'key', webSettings),
            group: BARK_GROUP,
            // 添加配置来源信息
            configSource: externalBarkConfig
        },
        auth: {
