...
install_deps() {
  if command -v apt-get >/dev/null 2>&1; then
    apt-get update -y
    apt-get install -y socat ipset iptables curl ca-certificates
  elif command -v yum >/dev/null 2>&1; then
    yum install -y epel-release || true
    yum install -y socat ipset iptables curl ca-certificates
  else
...
svc_name() { echo "${SERVICE_PREFIX}-$1.service"; }
set_name() { echo "${SET_PREFIX}_$1"; }
...
show_region_menu() {
  ...
  echo "请选择地区："
  for i in "${!codes[@]}"; do
    printf "%2d) %-4s %s\n" "$((i+1))" "${codes[$i]}" "${labels[$i]}"
  done
  while true; do
    read -r -p "请输入序号: " choice
    [[ "$choice" =~ ^[0-9]+$ ]] || { echo "请输入数字"; continue; }
    (( choice >= 1 && choice <= ${#codes[@]} )) || { echo "超出范围"; continue; }
    REGION_CHOICE="${codes[$((choice-1))]}"
    return 0
  done
}
...
add_flow() {
  ...
  ui_input "新增转发" "目标端口"
  dport="$UI_VALUE"
  if ui_yesno "新增转发" "是否启用地区白名单？"; then
    use_region="yes"
    show_region_menu
    region_code="$REGION_CHOICE"
  else
    use_region="no"
    region_code=""
  fi
